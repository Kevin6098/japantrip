    ...
    <div className="min-h-screen py-12 px-4 sm:px-6 max-w-6xl mx-auto pb-24">
      ...
      <div className="text-center mb-12 animate-fade-in">
        <div className="inline-flex items-center bg-white px-6 py-3 rounded-full shadow border border-blue-100 mb-4">
          ...
          <h1 className="font-header text-2xl font-bold text-slate-800">
            {t('Transportation Budget', '交通预算')}
          </h1>
        </div>
        <p className="text-sm text-slate-600 max-w-2xl mx-auto">
          {t('Detailed breakdown of train, bus, and walking times for each day', '每天的电车、巴士和步行时间详细分解')}
        </p>
      </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`text-lg font-bold ${getTotalColorClasses(day.color)}`}>
                      {typeof day.totalCost === 'object' 
                        ? t(day.totalCost.en, day.totalCost.zh)
                        : day.totalCost}
                    </span>
                  </div>
                </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="bg-blue-50 rounded-xl p-6 text-center">
            ...
            <p className="text-xs text-slate-600 mt-2">
              {t('*Excludes private vans (shared cost)', '*不含私人包车 (分摊费用)')}
            </p>
          </div>
          <div className="bg-green-50 rounded-xl p-6 text-center">
            <i className="fa-solid fa-walking text-green-600 text-3xl mb-3"></i>
            <h3 className="font-bold text-sm text-slate-700 mb-2">
              {t('Total Walking Time', '总步行时间')}
            </h3>
            ...
            <p className="text-xs text-slate-600 mt-2">
              {t('*Average ~61 mins per day', '*平均每天约61分钟')}
            </p>
          </div>
          <div className="bg-purple-50 rounded-xl p-6 text-center">
            ...
            <ul className="text-xs text-left text-slate-600 space-y-1">
              <li>• {t('Fares are estimates based on typical Japan transit costs', '费用基于日本典型交通成本估算')}</li>
              <li>• {t('Consider JR Pass for Day 3-11 if traveling extensively', '如广泛旅行，考虑第3-11天使用JR Pass')}</li>
              <li>• {t('IC cards (Suica/PASMO) recommended for convenience', '建议使用IC卡 (Suica/PASMO) 方便')}</li>
            </ul>
          </div>
        </div>
      </section>
      ...
      <section className="glass-card p-6 mt-8 animate-fade-in">
        <h2 className="font-header text-xl font-bold text-slate-800 mb-4">
          ...
          {t('Money-Saving Tips', '省钱提示')}
        </h2>
        <div className="grid md:grid-cols-2 gap-4">
          <div className="bg-yellow-50 rounded-lg p-4">
            <h3 className="font-bold text-sm text-slate-700 mb-2">
              {t('JR Pass Consideration', 'JR Pass考虑')}
            </h3>
            <p className="text-xs text-slate-600 mb-2">
              {t('7-day JR Pass: ~¥29,650. Your estimated JR costs (Days 4-11): ~¥22,000. May not be worth it unless you add more JR travel.', '7天JR Pass: 约¥29,650。您的JR估算费用 (第4-11天): 约¥22,000。除非增加更多JR旅行，否则可能不值得。')}
            </p>
          </div>
          <div className="bg-yellow-50 rounded-lg p-4">
            <h3 className="font-bold text-sm text-slate-700 mb-2">
            ...
            <p className="text-xs text-slate-600 mb-2">
              {t('Get Suica (Tokyo) or ICOCA (Osaka) for seamless travel. Works on most trains, buses, and convenience stores. Initial deposit: ¥500 (refundable).', '购买Suica (东京) 或ICOCA (大阪) 以便无缝旅行。适用于大多数电车、巴士和便利店。初始押金: ¥500 (可退还)。')}
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="mt-8 text-center">
        ...
        >
          ...
          <span>{t('← Back to Budget', '← 返回预算')}</span>
        </Link>
      </div>
    </div>
  )
